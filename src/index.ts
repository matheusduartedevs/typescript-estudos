...
function accelerate(targetSpeed:number, spaceship: { name:string, speed:number }) {
    if (spaceship.speed > targetSpeed) {
        alert(`Reduzindo a velociadade da ${spaceship.name} para ${targetSpeed}...`)
    } else if (spaceship.speed < targetSpeed) {
        alert(`Aumentando a velociadade da ${spaceship.name} para ${targetSpeed}...`)
    } else {
        alert(`Mantendo a velociadade da ${spaceship.name}`)
    }
}
